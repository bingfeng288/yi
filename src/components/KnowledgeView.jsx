...
import { LEVELS, DOMAIN_COLORS, knowledgeTree, FIRST_PRINCIPLES } from '../data/knowledgeTree';
...
function ExerciseItem({ ex, index, isCompleted, thought, onToggle, onSaveThought }) {
  ...
  return (
    <div className={`rounded-lg transition-all ${isCompleted ? 'bg-green-50 border border-green-200' : 'bg-ink-50'}`}>
      <div className="flex gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm leading-relaxed ${isCompleted ? 'text-ink-500 line-through' : 'text-ink-700'}`}>
            {ex}
          </p>
          ...
          <div className="flex justify-end gap-2 mt-1.5">
            {thoughtText && (
              <button
                ...
                className="text-[11px] text-ink-400 hover:text-red-500"
              >
                清除
              </button>
            )}
...
export default function KnowledgeView({ node, onNavigate, note, onUpdateNote, isBookmarked, onToggleBookmark, exerciseProgress, onToggleExercise, onSaveThought, scrollRef }) {
  ...
  const goToRelated = (relatedId) => {
    const target = topicMap[relatedId];
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fadeIn">
      ...
      <div className="flex items-center gap-2 text-sm text-ink-400 mb-4">
        ...
        <span>/</span>
        <span className="text-ink-700">{node.name}</span>
      </div>

      {/* Title section */}
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <h1 className="text-3xl font-bold text-ink-950">{node.name}</h1>
          <span
            className="yi-badge"
          ...
          <span className="yi-badge text-ink-400">⏱ {readingTime} 分钟</span>
          {onToggleBookmark && (
            <button
      ...
      <div id="kv-content" ref={registerRef('kv-content')} className="yi-card p-6 mb-6 scroll-mt-20">
        <h2 className="text-lg font-bold text-ink-950 mb-4 flex items-center gap-2">
          <span className="text-ink-300">📖</span> 概述
        </h2>
        <div className="text-ink-700 leading-relaxed text-base whitespace-pre-line space-y-3">
          {node.content?.split('\n\n').map((para, i) => (
            <p key={i}>{para}</p>
          ))}
        </div>
          ...
          <div className="space-y-3">
            {node.keyConcepts.map((kc, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  {typeof kc === 'string' ? (
                    <p className="text-ink-700 text-sm leading-relaxed">{kc}</p>
                  ) : (
                    <>
                      <p className="text-ink-950 text-sm font-bold">{kc.title}</p>
                      <p className="text-ink-600 text-sm leading-relaxed mt-0.5">{kc.desc}</p>
                    </>
                  )}
                </div>
              </div>
      ...
      {node.domain?.yi && (
        <div id="kv-yi" ref={registerRef('kv-yi')} className="yi-card p-5 mb-6 scroll-mt-20" style={{ borderLeft: `3px solid ${color.accent}` }}>
          <h3 className="text-sm font-bold text-ink-500 mb-1">
            周易之道 · {node.domain.yi}
          </h3>
      ...
      {node.domain?.firstPrinciple && (
        <div id="kv-principle" ref={registerRef('kv-principle')} className="yi-card p-5 mb-6 scroll-mt-20" style={{ borderLeft: `3px solid ${color.accent}` }}>
          <h3 className="text-sm font-bold text-ink-500 mb-1 flex items-center gap-1.5">
            <span>{FIRST_PRINCIPLES.icon}</span>
            第一性原理 · {node.domain.name}
          </h3>
          <p className="text-sm text-ink-600 mb-2">{node.domain.firstPrinciple}</p>
          <div className="mt-2 pt-2 border-t border-ink-100 space-y-1">
            {FIRST_PRINCIPLES.method.map((step, i) => (
              <p key={i} className="text-[11px] text-ink-400 leading-relaxed flex items-start gap-1.5">
      ...
      {related.length > 0 && (
        <div id="kv-refs" ref={registerRef('kv-refs')} className="yi-card p-5 mb-6 scroll-mt-20">
          <h3 className="text-sm font-bold text-ink-500 mb-3 flex items-center gap-2">
            <span className="text-ink-300">🔗</span> 跨学科关联
          </h3>
          <div className="space-y-2">
            {related.map((rel) => {
              const target = topicMap[rel.id];
              if (!target) return null;
              return (
                <button
                  key={rel.id}
                  onClick={() => goToRelated(rel.id)}
                  className="w-full text-left flex items-start gap-3 p-2.5 rounded-lg hover:bg-ink-50 transition-colors group"
                >
                  <span className="text-lg shrink-0">{target.domain.icon}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-ink-950 group-hover:text-ink-700">{target.name}</span>
                      <span className="text-[10px] text-ink-300">{target.domain.name}</span>
                    </div>
                    <p className="text-xs text-ink-400 mt-0.5">{rel.label}</p>
                  </div>
        ...
        <div id="kv-exercises" ref={registerRef('kv-exercises')} className="yi-card p-6 mb-6 scroll-mt-20">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-ink-400">{exDone}/{exTotal}</span>
            </div>
          </div>
          <div className="space-y-2">
      ...
      <div className="yi-card p-5 mb-6">
        <h3 className="text-sm font-bold text-ink-500 mb-3">难度层级</h3>
        <div className="flex gap-1.5">
          {Object.entries(LEVELS).map(([key, lv]) => (
            <div key={key} className="flex-1">
              ...
              />
              <div className={`text-[10px] text-center mt-1 ${Number(key) === node.level ? 'font-bold text-ink-950' : 'text-ink-300'}`}>
                {lv.name}
              </div>
            </div>
          ))}
      ...
      <div id="kv-notes" ref={registerRef('kv-notes')} className="yi-card p-5 mb-6 scroll-mt-20">
        ...
        >
          <h3 className="text-sm font-bold text-ink-500 flex items-center gap-2">
            ...
            <div className="flex justify-end gap-2 mt-2">
              {note && (
                <button
                  ...
                  className="yi-btn text-xs text-ink-400 hover:text-red-500"
                >
                  删除
                </button>
              )}
