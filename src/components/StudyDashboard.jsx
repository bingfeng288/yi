...
export default function StudyDashboard({ isOpen, onClose, tree, visited, bookmarks, notes, recent, unlocked, dailyVisits, dailyGoal, onSetDailyGoal, exerciseProgress, reviewStats, reviewData, onSelect, onImport }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto w-full max-w-lg bg-white h-full shadow-2xl flex flex-col animate-slideInLeft">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-ink-100">
          <h2 className="text-lg font-bold text-ink-950">学习中心</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-ink-100 text-ink-400">
          ...
          {tabs.map(t => (
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-medium transition-colors flex items-center justify-center gap-1.5
                ${activeTab === t.id ? 'text-ink-950 border-b-2 border-ink-950' : 'text-ink-400 hover:text-ink-600'}`}
            >
              <span>{t.icon}</span>
        ...
        <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="font-medium text-ink-950">总体进度</span>
                  <span className="text-ink-400">{visited.size}/{totalTopics} ({progressPct}%)</span>
                </div>
              ...
              {(() => {
                ...
                return (
                  <div className="grid grid-cols-5 gap-3">
                    {[
                      { label: '已学专题', value: visited.size, color: 'text-ink-950' },
                      ...
                      { label: '练习完成', value: exDone, color: 'text-green-600' },
                    ].map(s => (
                ...
                <div className="yi-card p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="text-lg font-bold text-ink-950">{calcStreak(dailyVisits)}</span>
                        <span className="text-xs text-ink-400 ml-1">天连续</span>
                      </div>
                    </div>
                        ...
                        <div key={day.date} className="flex-1 flex flex-col items-center gap-0.5">
                          <div
                            className={`w-full h-5 rounded-sm flex items-center justify-center text-[9px]
                              ...
                              ${day.isToday ? 'ring-1 ring-ink-950' : ''}`}
                          >
              ...
              {domainStats.length > 0 && (
                <div>
                  <h3 className="text-sm font-bold text-ink-950 mb-3">学科进度</h3>
                  <div className="space-y-2.5">
                    {domainStats.sort((a, b) => b.pct - a.pct).map(d => (
                      <div key={d.id}>
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="text-ink-600">{d.icon} {d.name}</span>
                          <span className="text-ink-400">{d.done}/{d.total}</span>
                        </div>
              ...
              {Object.keys(levelDist).length > 0 && (
                <div>
                  <h3 className="text-sm font-bold text-ink-950 mb-3">难度分布</h3>
                    ...
                    {Object.entries(LEVELS).map(([key, lv]) => {
                      ...
                      return (
                        <div key={key} className="flex-1 yi-card p-2 text-center">
                          ...
                          <div className="text-[10px] text-ink-400">{lv.name}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              {/* Export / Import */}
              <div>
                <h3 className="text-sm font-bold text-ink-950 mb-3">数据管理</h3>
                <div className="flex gap-2">
                  <button onClick={handleExport} className="yi-btn flex-1 px-3 py-2 rounded-lg border border-ink-200 text-sm text-ink-600 hover:bg-ink-50">
                    导出备份
                  </button>
                  <button onClick={() => fileRef.current?.click()} className="yi-btn flex-1 px-3 py-2 rounded-lg border border-ink-200 text-sm text-ink-600 hover:bg-ink-50">
                    导入恢复
                  </button>
                  <input ref={fileRef} type="file" accept=".json" onChange={handleImport} className="hidden" />
                </div>
                {importMsg && <p className="text-xs text-green-600 mt-2">{importMsg}</p>}
                <p className="text-[10px] text-ink-300 mt-2">导出为 JSON 文件，可在其他设备导入恢复</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'graph' && (
            <div>
              <p className="text-xs text-ink-400 mb-3">学科之间的跨领域关联网络，圆环进度表示已学比例</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <p className="text-sm text-ink-600">
                  已解锁 <span className="font-bold text-ink-950">{unlocked ? unlocked.size : 0}</span> / {ACHIEVEMENTS.length}
                </p>
                <div className="h-1.5 flex-1 max-w-[120px] bg-ink-100 rounded-full overflow-hidden ml-3">
                  <div
                ...
                {ACHIEVEMENTS.map(a => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-ink-400 leading-relaxed">{a.desc}</p>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-medium text-sm text-ink-950 group-hover:text-ink-700">{t.name}</span>
                          <span className="text-[10px] text-ink-300 ml-auto">
                            {note.updatedAt ? new Date(note.updatedAt).toLocaleDateString('zh-CN') : ''}
                          </span>
                        </div>
                        <p className="text-xs text-ink-500 line-clamp-2 leading-relaxed">{note.text}</p>
                      </button>
                    );
                  })}
                </div>
...
function EmptyState({ icon, text, hint }) {
  return (
    <div className="text-center py-12">
      ...
      <p className="text-ink-400">{text}</p>
